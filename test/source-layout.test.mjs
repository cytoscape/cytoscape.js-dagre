...
// 2. Safely extract the function, no matter how TS/Node wraps it
const register = typeof plugin === 'function' ? plugin
  : typeof plugin.default === 'function' ? plugin.default
  : typeof plugin.register === 'function' ? plugin.register
  : (plugin.default && typeof plugin.default.default === 'function') ? plugin.default.default
  : null;

// 3. If it STILL fails, print exactly what the module looks like so we can fix it
if (typeof register !== 'function') {
  console.error("DEBUG - What did we actually import?", plugin);
  throw new Error("Could not find a valid register function from src/index.ts");
}
