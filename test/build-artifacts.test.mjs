...
describe('built artifacts', function(){
  it('keeps legacy UMD artifact links pointed at dist', function(){
    ...
    // Helper function to handle both true symlinks and Windows Git text files
    const verifyLink = (linkPath, expectedTarget) => {
      const stats = fs.lstatSync( linkPath );
      
      if ( stats.isSymbolicLink() ) {
        // Mac/Linux/Windows with Developer Mode
        let target = path.resolve( path.dirname( linkPath ), fs.readlinkSync( linkPath ) );
        expect( target ).to.equal( expectedTarget );
      } else {
        // Windows fallback: Git checks out symlinks as text files containing the path
        let content = fs.readFileSync( linkPath, 'utf8' ).trim();
        let target = path.resolve( path.dirname( linkPath ), content );
        expect( target ).to.equal( expectedTarget );
      }
    };

    verifyLink( rootLink, path.resolve( dist, 'cytoscape-dagre.js' ) );
    verifyLink( pagesLink, path.resolve( dist, 'cytoscape-dagre.js' ) );
  });
