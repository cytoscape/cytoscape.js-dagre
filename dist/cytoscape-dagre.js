...
(function(global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define([], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.cytoscapeDagre = factory());
})(this, function() {
	...
	*/
	...
	var bn = Object.defineProperty, Le = (e, n) => {
		...
		nodeEdges(e, n) {
			if (e in this._nodes) return this.filterEdges(_objectSpread2(_objectSpread2({}, this._in[e]), this._out[e]), e, n);
		}
	...
	function Nn(e, n, t, r) {
		let o = {}, i, s = 0, a = e.nodes(), d = function(c) {
			let h = t(c);
			o[c.v].distance + h < o[c.w].distance && (o[c.w] = {
				distance: o[c.v].distance + h,
				predecessor: c.v
			}, i = !0);
		}, l = function() {
	...
	var Ie = 65535;
	function L(e, n) {
		if (n.length > Ie) return e(...Dn(n).map((r) => e(...r)));
		else return e(...n);
	}
	function X(e) {
		let t = e.nodes().map((r) => {
	...
	function je(e) {
		(e.graph().acyclicer === "greedy" ? Q(e, t(e)) : Hn(e)).forEach((r) => {
			let o = e.edge(r);
			e.removeEdge(r), o.forwardName = r.name, o.reversed = !0, e.setEdge(r.w, r.v, o, j("rev"));
		});
		function t(r) {
	...
	function Hn(e) {
		let n = [], t = {}, r = {};
		function o(i) {
			Object.hasOwn(r, i) || (r[i] = !0, t[i] = !0, e.outEdges(i).forEach((s) => {
	...
	function Xn(e, n) {
		...
		for (c = 0, ++r; r < i; ++c, ++r) a.points = [], u = {
			...
			edgeObj: n,
			rank: r
		}, l = w(e, "edge", u, "_d"), r === d && (u.width = a.width, u.height = a.height, u.dummy = "edge-label", u.labelpos = a.labelpos), e.setEdge(t, l, { weight: a.weight }, s), c === 0 && e.graph().dummyChains.push(l), t = l;
	...
	var V = Kn;
	function Kn(e) {
		let n = new p({ directed: !1 }), t = e.nodes();
	...
	var { preorder: Zn, postorder: et } = R, Ve = x;
	...
	function ze(e, n, t) {
		...
		return i.lim > s.lim && (a = s, d = !0), n.edges().filter((u) => d === Ae(e, e.node(u.v), a) && d !== Ae(e, e.node(u.w), a)).reduce((u, c) => v(n, c) < v(n, u) ? c : u);
	}
	function He(e, n, t, r) {
		let o = t.v, i = t.w;
	...
	function at(e) {
		...
		e.graph().dummyChains.forEach((t) => {
			let r = e.node(t), o = r.edgeObj, i = dt(e, n, o.v, o.w), s = i.path, a = i.lca, d = 0, l = s[d], u = !0;
			for (; t !== o.w;) {
				if (r = e.node(t), u) {
	...
	function re(e) {
		let n = {}, t = e.nodes().filter((d) => !e.children(d).length), r = t.map((d) => e.node(d).rank), i = k(L(Math.max, r) + 1).map(() => []);
		function s(d) {
			if (n[d]) return;
			n[d] = !0;
	...
	function se(e, n) {
		...
		return Et(Object.values(t).filter((o) => !o.indegree));
	}
	function Et(e) {
		let n = [];
		function t(o) {
		...
		return n.filter((o) => !o.merged).map((o) => T(o, [
			"vs",
			"i",
			"barycenter",
			"weight"
	...
	function de(e, n, t, r) {
		r || (r = e.nodes());
		let o = Gt(e), i = new p({ compound: !0 }).setGraph({ root: o }).setDefaultNodeLabel((s) => e.node(s));
		return r.forEach((s) => {
	...
	function ln(e) {
		...
		return Pt(r, Rt(e, r)), Mt(r, e.graph().align);
	}
	function jt(e, n, t) {
		return (r, o, i) => {
	...
	function dagreEdgeToCytoscapeEdge(dEdge, cEdge) {
		...
		const coords = normalizeWeight(((_dEdge$points = dEdge.points) !== null && _dEdge$points !== void 0 ? _dEdge$points : []).map((p) => toEdgeCoordinates(p, frame)));
		return {
	...
	DagreLayout.prototype.run = function() {
		var _options$boundingBox;
		const options = this.options;
		const layout = this;
		const cy = options.cy;
		const eles = options.eles;
		const getVal = function(ele, val) {
			return isFunction(val) ? val.call(ele, ele) : val;
		};
		const bb = completeBoundingBox((_options$boundingBox = options.boundingBox) !== null && _options$boundingBox !== void 0 ? _options$boundingBox : {
			x1: 0,
			y1: 0,
			w: cy.width(),
			h: cy.height()
		...
		const gObj = {};
		const setGObj = function(name, val) {
		...
		for (let i = 0; i < nodes.length; i++) {
			...
			if (node.isChild()) {
				...
				if (parent) g.setParent(node.id(), parent.id());
			}
		}
		let edges = eles.edges().filter(function(edge) {
			return !edge.source().isParent() && !edge.target().isParent();
		});
		if (isFunction(options.sort)) edges = edges.sort(options.sort);
		for (let i = 0; i < edges.length; i++) {
			var _options$minLen, _options$edgeWeight;
			let edge = edges[i];
			g.setEdge(edge.source().id(), edge.target().id(), {
				minlen: getVal(edge, (_options$minLen = options.minLen) !== null && _options$minLen !== void 0 ? _options$minLen : defaults.minLen),
				weight: getVal(edge, (_options$edgeWeight = options.edgeWeight) !== null && _options$edgeWeight !== void 0 ? _options$edgeWeight : defaults.edgeWeight),
				name: edge.id()
			}, edge.id());
		}
		To.layout(g);
		...
		nodes.layoutPositions(layout, options, function(ele) {
			...
			if (!dModel || !hasPosition(dModel)) return ele.position();
			return constrainPos({
				x: dModel.x,
				y: dModel.y
			});
		...
		if (options.useDagreEdgeControlPoints) {
			if (options.automaticDagreEdgeStyle) {
				var _options$dagreEdgeSty;
				cy.edges().addClass("useDagreEdgeControlPoints");
				cy.style().selector("edge.useDagreEdgeControlPoints").style((_options$dagreEdgeSty = options.dagreEdgeStyle) !== null && _options$dagreEdgeSty !== void 0 ? _options$dagreEdgeSty : defaults.dagreEdgeStyle).update();
			}
			g.edges().forEach((id) => {
				if (id.name === void 0) return;
				const cyEdge = cy.getElementById(id.name);
				const dEdge = g.edge(id);
				if (dEdge && dEdge.points) Object.assign(getScratch(cyEdge), dagreEdgeToCytoscapeEdge(dEdge, cyEdge));
			});
		}
		return this;
	};
	//#endregion
	//#region src/index.ts
	const register = function(cytoscapeLib) {
		if (!cytoscapeLib) return;
		cytoscapeLib("layout", "dagre", DagreLayout);
	};
	if (typeof window !== "undefined" && "cytoscape" in window) register(window.cytoscape);
	//#endregion
	return register;
});
