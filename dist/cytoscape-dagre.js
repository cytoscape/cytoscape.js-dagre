...
/**
	...
	*/
	function normalizeWeight(coords) {
		...
		for (const point of coords) {
			if (point.weight < min) min = point.weight;
			if (point.weight > max) max = point.weight;
		}
	...
	*/
	function dagreEdgeToCytoscapeEdge(dEdge, cEdge) {
		...
		const coords = normalizeWeight((dEdge.points || []).map((point) => toEdgeCoordinates(point, frame)));
		return {
			controlPointWeights: coords.slice(1, -1).map((coordinate) => coordinate.weight),
			controlPointDistances: coords.slice(1, -1).map((coordinate) => coordinate.distance)
		};
	}
	/**
	...
	DagreLayout.prototype.run = function() {
		...
		*/
		...
		for (let i = 0; i < edges.length; i++) {
			...
			*/
			if (edge.source().isParent() || edge.target().isParent()) continue;
			/**
...
//#region src/index.ts
	...
	if (typeof window !== "undefined" && typeof window.cytoscape !== "undefined") register(window.cytoscape);
